...
public class RemoveController implements Initializable{
	...
	public void removeButtonOnAction(ActionEvent event) throws Exception{
		if (typesBox.getValue() != null &&
			...
			switch(type) {
				case "Income":
					isRemoved = IncomeModel.removeIncome(username, category, amount, date, detail);
					
					if(isRemoved) {
						removeMessage.setText("Income Removed");
					} else {
						removeMessage.setText("Income not found");
					}
					
					break;
				case "Expense":
					isRemoved = ExpenseModel.removeExpense(username, category, amount, date, detail);
					
					if (isRemoved) {
						removeMessage.setText("Expense Removed");
					} else {
						removeMessage.setText("Expense not found");
					}
